    ...
    <div class="container mt-2">
        <div class="row">
            <div class="col-lg-12 margin-tb">
                ...
                <div class="pull-right">
                    <a class="btn btn-primary" href="{{ route('list_role') }}"> Back</a>
                </div>
            </div>
        </div>
        @if(session('status'))
        ...
        <form action="{{ route('role_store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                <input type="checkbox" name="perms[]" value="{{$permission->id}}" checked><span>{{$permission->title}}</span><span class="checkmark"></span>
                </label>
                </div>
                @endforeach
                </div>
